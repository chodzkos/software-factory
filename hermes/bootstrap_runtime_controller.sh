...
REVIEWER_VERIFY_SRC="${ROOT_DIR}/hermes/verify_reviewer_capabilities.py"
PROFILE_VERIFY_SRC="${ROOT_DIR}/hermes/verify_profile_capabilities.py"
CAPABILITY_PROBE_SRC="${ROOT_DIR}/hermes/capability_surface_probe.py"
...
for path in "${SOUL_SRC}" "${WRAPPER_SRC}" "${VALIDATOR_SRC}" "${MODEL_ROUTING_SRC}" "${REVIEW_DISPATCH_SRC}" "${REVIEWER_VERIFY_SRC}" "${PROFILE_VERIFY_SRC}" "${CAPABILITY_PROBE_SRC}" "${HANDOFF_SRC}" "${PLUGIN_INSTALLER}" "${CONFIG_KEY_REMOVER}"; do test -f "${path}" || { echo "ERROR: missing ${path}" >&2; exit 1; }; done
...
install -m 0644 "${REVIEWER_VERIFY_SRC}" "${PROFILE_DIR}/verify_reviewer_capabilities.py"
install -m 0644 "${PROFILE_VERIFY_SRC}" "${PROFILE_DIR}/verify_profile_capabilities.py"
install -m 0644 "${CAPABILITY_PROBE_SRC}" "${PROFILE_DIR}/capability_surface_probe.py"
...
test -f "${PROFILE_DIR}/verify_reviewer_capabilities.py"
test -f "${PROFILE_DIR}/verify_profile_capabilities.py"
test -f "${PROFILE_DIR}/capability_surface_probe.py"
