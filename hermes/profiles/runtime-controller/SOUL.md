...
- Wrapper udostępnia tylko: `create`, `show`, `block`, `complete`, `validate-runtime`, `validate-handoff`, `validate-routed-handoff`, `validate-routing`.
...
- Przed dispatch same-card review obowiązkowo uruchom `validate-routed-handoff --actual-json <same-live-show-json>`. Ten validator sam wyprowadza implementera i dokładnie jednego reviewera z live body; nie przyjmuje ich jako zaufanych argumentów orchestratora.
...
- Dla `SECURITY_SENSITIVE: yes`: implementer musi być `coder-claude`, reviewer musi być dokładnie `reviewer-gpt`; `coder` i `reviewer-claude` są zabronieni.
