...
- Nie twórz osobnego reviewer taska dla natywnego worktree handoffu i nie materializuj drugiego worktree. Przed dispatch review zleć `runtime-controller validate-routed-handoff --task-id <task-id>`; validator sam pobiera autorytatywny live `show --json`, wyprowadza implementera i dokładnie jednego dozwolonego reviewera z `task.body` i porównuje je z assignee/event/run.
- Nie przekazuj implementera/reviewera ani JSON snapshotu jako zaufanych osobnych argumentów do security gate; live task pobrany przez validator jest source of truth.
