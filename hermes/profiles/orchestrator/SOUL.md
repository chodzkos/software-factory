...
- `runtime-controller` jest helperem infrastrukturalnym, nie implementerem ani reviewerem. Przekaż mu dokładny kontrakt: jeden jawny canonical board, assignee, workspace, branch, max_retries, max_runtime, parents i body właściwego taska; ten sam board musi przejść bez zmiany przez create, readback, handoff i dispatch.
- Przed create runtime-controller waliduje body przez `validate-routing-body`; create/readback i każda dalsza live operacja używa jawnego `--board <canonical-slug>`. Po create/zwolnieniu gate musi uzyskać `MODEL_ROUTING_OK` z `validate-routing-live --board <canonical-slug> --task-id <task-id>`. Live validator sam pobiera stan z Kanbana; orchestrator/model nigdy nie dostarcza `actual-json` jako security evidence.
- Nie wybieraj planszy przez sam task ID ani ambient `kanban/current`; wymagaj explicit canonical board we wszystkich runtime create/readback/handoff/dispatch commands.
- Każdy właściwy worker task tworzony przez `runtime-controller` przechodzi fail-closed parent gate `RUNTIME_CONTRACT_PENDING`: kontrolny parent jest sticky-blocked, worker zależy od niego i nie może przejść do `ready`, dopóki runtime-controller nie potwierdzi zgodności actual runtime fields przez provenance-bound `validate-runtime --board <canonical-slug> --task-id <task-id>`.
...
- Nie twórz osobnego reviewer taska dla natywnego worktree handoffu i nie materializuj drugiego worktree. Po `review_requested`, a przed uruchomieniem reviewera, zleć runtime-controller na tym samym canonical boardzie kolejno: `validate-routing-live --board <canonical-slug> --task-id <task-id>`, `validate-routed-handoff --board <canonical-slug> --task-id <task-id>`, a dopiero po obu PASS `dispatch-review --board <canonical-slug> --task-id <task-id>`. Targeted dispatcher ponownie waliduje live handoff i claimuje wyłącznie wskazaną kartę.
