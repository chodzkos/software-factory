...
class RuntimeContractTests(unittest.TestCase):
    ...
    def test_exact_resolved_worktree_shape_and_declared_repo(self):
        ...
        self.assertEqual(resolved_implementation_worktree(payload), "/repo/.worktrees/t_impl")
        bads = (
    ...
    def test_run_metadata_workspace_and_task_id_are_mandatory(self):
        ...
        payload=same_card_review_snapshot(); payload["runs"][0]["metadata"]["task_id"]="other"
        self.assertIn("implementer_review_run_task_mismatched", validate_review_handoff(payload, implementer_profile="coder", reviewer_profile="reviewer-claude"))
    ...
    def test_runtime_cli_returns_zero_and_two(self):
        ...
        out=io.StringIO()
        with contextlib.redirect_stdout(out): rc=main(["runtime","--actual-json",actual,"--assignee","coder","--workspace-kind","worktree","--workspace-path","/repo","--branch-name","pilot/x","--max-retries","1"])
        self.assertEqual(rc,0); self.assertEqual(out.getvalue().strip(),"RUNTIME_CONTRACT_OK")
        out=io.StringIO()
        with contextlib.redirect_stdout(out): rc=main(["runtime","--actual-json",actual,"--assignee","other","--workspace-kind","worktree","--workspace-path","/repo"])
