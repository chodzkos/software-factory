...
def snapshot(*, body: str, assignee: str, event_reviewer: str) -> dict:
    return {
        "task": {
            ...
            "workspace_path": "/repo/.worktrees/t_route", "body": body,
        },
        "events": [{"kind": "review_requested", "payload": {"implementer": "coder", "reviewer": event_reviewer}, "run_id": 21}],
        "runs": [{"id": 21, "profile": "coder", "outcome": "review_requested", "metadata": {"workspace_path": "/repo/.worktrees/t_route", "task_id": "t_route"}}],
    }


class RoutedHandoffTests(unittest.TestCase):
    def test_declared_cross_vendor_handoff_passes(self):
        self.assertEqual(validate_routed_review_handoff(snapshot(body=NORMAL, assignee="reviewer-claude", event_reviewer="reviewer-claude")), [])

    def test_actual_reviewer_cannot_differ_from_task_body(self):
        errors = validate_routed_review_handoff(snapshot(body=NORMAL, assignee="reviewer-gpt", event_reviewer="reviewer-gpt"))
    ...
    def test_security_openai_implementer_is_rejected_before_handoff(self):
        body = NORMAL.replace("SECURITY_SENSITIVE: no", "SECURITY_SENSITIVE: yes").replace("REQUIRED_REVIEWERS: reviewer-claude", "REQUIRED_REVIEWERS: reviewer-gpt")
        errors = validate_routed_review_handoff(snapshot(body=body, assignee="reviewer-gpt", event_reviewer="reviewer-gpt"))
    ...
    def test_extra_declared_reviewer_is_rejected_before_handoff(self):
        body = NORMAL.replace("REQUIRED_REVIEWERS: reviewer-claude", "REQUIRED_REVIEWERS: reviewer-claude,reviewer-gpt")
        errors = validate_routed_review_handoff(snapshot(body=body, assignee="reviewer-claude", event_reviewer="reviewer-claude"))
    ...
    def test_run_metadata_workspace_and_task_are_mandatory(self):
        payload = snapshot(body=NORMAL, assignee="reviewer-claude", event_reviewer="reviewer-claude"); payload["runs"][0]["metadata"] = None
        self.assertIn("implementer_review_run_metadata_required", validate_routed_review_handoff(payload))
        payload = snapshot(body=NORMAL, assignee="reviewer-claude", event_reviewer="reviewer-claude"); payload["runs"][0]["metadata"] = {"workspace_path": "/repo/.worktrees/t_other", "task_id": "t_route"}
        self.assertIn("implementer_review_run_workspace_mismatched", validate_routed_review_handoff(payload))
        payload = snapshot(body=NORMAL, assignee="reviewer-claude", event_reviewer="reviewer-claude"); payload["runs"][0]["metadata"]["task_id"] = "other"
        self.assertIn("implementer_review_run_task_mismatched", validate_routed_review_handoff(payload))

    def test_lexical_or_wrong_repo_worktree_is_rejected(self):
        for path in ("/repo/.worktrees/t_route/../../escape", "/other/.worktrees/t_route"):
            payload = snapshot(body=NORMAL, assignee="reviewer-claude", event_reviewer="reviewer-claude")
            payload["task"]["workspace_path"] = path
            payload["runs"][0]["metadata"]["workspace_path"] = path
    ...
    def test_duplicate_json_keys_fail_closed_in_routed_cli(self):
        ...
        forbidden = NORMAL.replace("SECURITY_SENSITIVE: no", "SECURITY_SENSITIVE: yes").replace("REQUIRED_REVIEWERS: reviewer-claude", "REQUIRED_REVIEWERS: reviewer-gpt")
        raw = (
        ...
        with self.assertRaises(SystemExit) as ctx:
            with contextlib.redirect_stdout(io.StringIO()): main(["routed-handoff", "--actual-json", raw])
