...
"""
    ...
    def test_declared_cross_vendor_handoff_passes(self):
        self.assertEqual(validate_routed_review_handoff(self.snapshot(), board="isolated"), [])

    def test_actual_reviewer_cannot_differ_from_task_body(self):
        errors = validate_routed_review_handoff(self.snapshot(assignee="reviewer-gpt", event_reviewer="reviewer-gpt"), board="isolated")
    ...
    def test_security_openai_implementer_is_rejected_before_handoff(self):
        ...
        errors = validate_routed_review_handoff(self.snapshot(body=body, assignee="reviewer-gpt", event_reviewer="reviewer-gpt"), board="isolated")
    ...
    def test_extra_declared_reviewer_is_rejected_before_handoff(self):
        ...
        errors = validate_routed_review_handoff(self.snapshot(body=body), board="isolated")
    ...
    def test_event_run_id_is_mandatory_and_boolean_rejected(self):
        ...
        errors = validate_routed_review_handoff(payload, board="isolated")
        ...
        self.assertIn("review_requested_event_run_id_required", validate_routed_review_handoff(payload, board="isolated"))

    def test_run_metadata_workspace_and_task_are_mandatory(self):
        payload = self.snapshot(); payload["runs"][0]["metadata"] = None
        self.assertIn("implementer_review_run_metadata_required", validate_routed_review_handoff(payload, board="isolated"))
        payload = self.snapshot(); payload["runs"][0]["metadata"] = {"workspace_path": str(self.repo / ".worktrees" / "t_other"), "task_id": "t_route"}
        self.assertIn("implementer_review_run_workspace_mismatched", validate_routed_review_handoff(payload, board="isolated"))
        payload = self.snapshot(); payload["runs"][0]["metadata"]["task_id"] = "other"
        self.assertIn("implementer_review_run_task_mismatched", validate_routed_review_handoff(payload, board="isolated"))
    ...
    def test_lexical_nonexistent_symlink_or_wrong_repo_worktree_is_rejected(self):
        ...
        for path in candidates:
            ...
            self.assertIn("implementation_resolved_worktree_missing", validate_routed_review_handoff(payload, board="isolated"))
