...
MAP_SCHEMA = {
    "name": "factory_repo_map",
    "description": "Map source files inside the current Kanban workspace. Workspace is fixed by Hermes and cannot be supplied by the caller.",
    "parameters": {
        "type": "object",
        "properties": {
            "target": {"type": "string", "description": "Workspace-relative directory, default '.'."},
        },
        "additionalProperties": False,
    },
}
READ_SCHEMA = {
    "name": "factory_repo_read",
    "description": "Read a bounded UTF-8 text file inside the current Kanban workspace. Secret-like files, symlinks, binaries, and paths outside the workspace are refused.",
    "parameters": {
        "type": "object",
        "properties": {
            "path": {"type": "string", "description": "Workspace-relative file path."},
            "start_line": {"type": "integer", "minimum": 1, "description": "First line, default 1."},
            "max_lines": {"type": "integer", "minimum": 1, "maximum": 400, "description": "Maximum lines returned, default 200."},
        },
        "required": ["path"],
        "additionalProperties": False,
    },
...
SEARCH_SCHEMA = {
    "name": "factory_repo_search",
    "description": "Literal text search over bounded non-secret text files inside the current Kanban workspace. No regex or executable search command is exposed.",
    "parameters": {
        "type": "object",
        "properties": {
            "query": {"type": "string", "description": "Literal text to find (1-256 chars)."},
            "target": {"type": "string", "description": "Workspace-relative directory, default '.'."},
            "case_sensitive": {"type": "boolean", "description": "Default false."},
            "max_results": {"type": "integer", "minimum": 1, "maximum": 100, "description": "Maximum matching lines, default 50."},
        },
        "required": ["query"],
        "additionalProperties": False,
    },
...
def _secret_like(path: Path) -> bool:
    ...
    if lower.startswith(".env") or lower in repo_map.SECRET_NAMES:
        return True
    if path.suffix.lower() in repo_map.SECRET_SUFFIXES:
        return True
    if any(token in lower for token in ("credential", "secret", "private_key", "access_token")):
        return True
    return False
...
def _validate_relative(raw: str, *, allow_dot: bool) -> PurePosixPath:
    ...
    if not meaningful and not allow_dot:
        raise ValueError("file path required")
    for part in meaningful[:-1] if not allow_dot else meaningful:
        if part in SKIP_DIRS or (part.startswith(".") and part not in SAFE_HIDDEN_DIRS):
            raise ValueError("hidden/generated path component refused")
    return p
...
def _resolve_inside(workspace: Path, raw: str, *, expect_dir: bool) -> Path:
    ...
    try:
        resolved = (workspace / Path(*rel.parts)).resolve(strict=True)
        resolved.relative_to(workspace)
    except (OSError, ValueError) as exc:
        raise ValueError("path unavailable or outside workspace") from exc
    if expect_dir and not resolved.is_dir():
        raise ValueError("target must be a directory")
    if not expect_dir and (not resolved.is_file() or resolved.is_symlink()):
...
def _read_utf8(path: Path, max_bytes: int) -> tuple[str, int]:
    ...
    try:
        ...
        if len(data) > max_bytes or b"\x00" in data:
            raise ValueError("binary or oversized file refused")
        try:
...
def _is_text_candidate(path: Path) -> bool:
    lower = path.name.lower()
    return lower in TEXT_NAMES or path.suffix.lower() in TEXT_EXTENSIONS
...
def handle_map(args: dict, **kw) -> str:
    try:
        ...
        _resolve_inside(workspace, target, expect_dir=True)
        buffer = io.StringIO()
        argv = ["--workspace", str(workspace), *MAP_LIMITS, "--", target]
        with redirect_stdout(buffer):
            rc = repo_map.main(argv)
        if rc != 0:
            return tool_error("repository map failed")
        return tool_result({"workspace_bound": True, "map": buffer.getvalue()})
...
def handle_read(args: dict, **kw) -> str:
    try:
        ...
        path = _resolve_inside(workspace, raw, expect_dir=False)
        if _secret_like(path):
            raise ValueError("secret-like file refused")
        content, size = _read_utf8(path, MAX_READ_BYTES)
        start = max(1, int(args.get("start_line") or 1))
        max_lines = max(1, min(MAX_READ_LINES, int(args.get("max_lines") or 200)))
        lines = content.splitlines()
        shown = lines[start - 1 : start - 1 + max_lines]
        rel = path.relative_to(workspace).as_posix()
        return tool_result({
            "path": repo_map.sanitize(rel),
            "start_line": start,
            "end_line": start + len(shown) - 1 if shown else start - 1,
            "total_lines": len(lines),
            "bytes": size,
            "truncated": start - 1 + len(shown) < len(lines),
            "content": "\n".join(shown),
        })
...
def handle_search(args: dict, **kw) -> str:
    try:
        ...
        if not query or len(query) > MAX_QUERY_CHARS or "\x00" in query:
            raise ValueError("query must contain 1-256 non-NUL characters")
        target = _resolve_inside(workspace, str(args.get("target") or "."), expect_dir=True)
        case_sensitive = bool(args.get("case_sensitive", False))
        max_results = max(1, min(MAX_SEARCH_RESULTS, int(args.get("max_results") or 50)))
        needle = query if case_sensitive else query.casefold()
        results: list[dict[str, Any]] = []
        files_seen = dirs_seen = total_bytes = 0
        truncated = False
        stack = [target]
        while stack and len(results) < max_results:
            current = stack.pop()
            dirs_seen += 1
            if dirs_seen > MAX_SEARCH_DIRS:
                truncated = True
                break
            ...
            for entry in entries:
                ...
                try:
                    ...
                    if entry.is_dir(follow_symlinks=False):
                        if entry.name in SKIP_DIRS or (entry.name.startswith(".") and entry.name not in SAFE_HIDDEN_DIRS):
                            continue
                        resolved_dir = path.resolve(strict=True)
                        resolved_dir.relative_to(workspace)
                ...
                if files_seen > MAX_SEARCH_FILES:
                    ...
                    stack.clear()
                    break
                if _secret_like(path) or not _is_text_candidate(path):
                    continue
                try:
                    info = entry.stat(follow_symlinks=False)
                except OSError:
                    continue
                if not stat.S_ISREG(info.st_mode) or info.st_size > MAX_SEARCH_FILE_BYTES:
                    continue
                if total_bytes + info.st_size > MAX_SEARCH_TOTAL_BYTES:
                    truncated = True
                    stack.clear()
                    break
                try:
                    resolved = path.resolve(strict=True)
                    resolved.relative_to(workspace)
                    content, size = _read_utf8(resolved, MAX_SEARCH_FILE_BYTES)
                except (OSError, ValueError):
                    continue
                total_bytes += size
                per_file = 0
                for lineno, line in enumerate(content.splitlines(), 1):
                    ...
                    if needle in haystack:
                        results.append({
                            "path": repo_map.sanitize(resolved.relative_to(workspace).as_posix()),
                            "line": lineno,
                            "text": repo_map.sanitize(line[:500]),
                        })
                        per_file += 1
