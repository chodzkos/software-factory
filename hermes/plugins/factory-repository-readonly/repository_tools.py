...
ALLOWED_PROFILES = frozenset({"repository-analyst", "reviewer-gpt", "release-manager"})
...
def _bound_workspace() -> Path:
    ...
    if profile not in ALLOWED_PROFILES: raise ValueError("authorized read-only profile required")
    if profile == "release-manager":
        if not os.environ.get("FACTORY_RELEASE_TASK", "").strip(): raise ValueError("missing release-decision task binding")
        raw = os.environ.get("FACTORY_RELEASE_WORKSPACE", "").strip()
    else:
    ...
    if not raw: raise ValueError("missing bound workspace")
    path = Path(raw)
