...
Wrapper wyprowadza dokładny Hermes-managed Python ze zweryfikowanego launchera, a przed uruchomieniem helpera czyści `PYTHONPATH`, `PYTHONHOME`, `PYTHONSTARTUP` i `PYTHONINSPECT` oraz używa `-B -E -s`. Probe interpretera używa `-B -I`; jawne `-B` zapobiega zapisowi bytecode także wtedy, gdy `-E` lub `-I` ignoruje `PYTHONDONTWRITEBYTECODE`. Nie istnieje chroniona operacja board-globalnego review dispatchu.
