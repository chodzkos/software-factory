...
def normalize_snapshot(payload: Mapping[str, Any]) -> dict[str, Any]:
    if "task" in payload:
        ...
        if not isinstance(nested, Mapping): return {}
        task = dict(nested)
        if "parents" not in task and "parents" in payload: task["parents"] = payload.get("parents")
    else:
        task = dict(payload)
    if "parents" not in task and "parent_ids" in task: task["parents"] = task.get("parent_ids")
    return task
...
def _records(payload: Mapping[str, Any], key: str) -> tuple[Mapping[str, Any], ...]:
    ...
    if any(not isinstance(value, Mapping) for value in raw): return ()
    return tuple(raw)
...
def _latest_review_requested_event(payload: Mapping[str, Any]) -> Mapping[str, Any] | None:
    for event in reversed(_records(payload, "events")):
        if event.get("kind") == "review_requested": return event
    return None
...
def validate_runtime(payload: Mapping[str, Any], expectation: RuntimeExpectation) -> list[str]:
    ...
    if not actual: return ["runtime_task_missing_or_invalid"]
    errors: list[str] = []
    checks = {"assignee": expectation.assignee, "workspace_kind": expectation.workspace_kind, "workspace_path": expectation.workspace_path, "branch_name": expectation.branch_name, "max_retries": expectation.max_retries}
    for field, expected in checks.items():
        if expected is not None and actual.get(field) != expected: errors.append(f"{field}: expected={expected!r} actual={actual.get(field)!r}")
    if _parents(actual) != expectation.parents: errors.append(f"parents: expected={expectation.parents!r} actual={_parents(actual)!r}")
    return errors


def _canonical_absolute(raw: str) -> str | None:
    if not isinstance(raw, str) or not raw.startswith("/"): return None
    pure = PurePosixPath(raw)
    if any(part in {".", ".."} for part in pure.parts): return None
    path = Path(raw)
    if path.exists():
        ...
        try:
            ...
            return None
        if resolved != raw.rstrip("/"): return None
    return raw.rstrip("/")
...
def _declared_repository_root(task: Mapping[str, Any]) -> str | None:
    ...
    if len(values) != 1 or not values[0].startswith("worktree:"): return None
...
def resolved_implementation_worktree(payload: Mapping[str, Any]) -> str | None:
    ...
    if task.get("workspace_kind") != "worktree": return None
    task_id = task.get("id"); path = task.get("workspace_path")
    if not isinstance(task_id, str) or not task_id or not isinstance(path, str): return None
    repo_root = _declared_repository_root(task)
    canonical_path = _canonical_absolute(path)
    if not repo_root or not canonical_path: return None
...
def validate_review_handoff(payload: Mapping[str, Any], *, implementer_profile: str, reviewer_profile: str) -> list[str]:
    task = normalize_snapshot(payload)
    if not task: return ["implementation_task_missing_or_invalid"]
    errors: list[str] = []
    if implementer_profile == reviewer_profile: errors.append("implementer_and_reviewer_must_differ")
    task_id = task.get("id")
    if not isinstance(task_id, str) or not task_id:
        errors.append("implementation_id_missing"); return errors
    resolved_path = resolved_implementation_worktree(payload)
    if not resolved_path:
        errors.append("implementation_resolved_worktree_missing"); return errors
    if task.get("assignee") != reviewer_profile: errors.append(f"review_assignee: expected={reviewer_profile!r} actual={task.get('assignee')!r}")
    if task.get("status") != "review": errors.append(f"review_status: expected='review' actual={task.get('status')!r}")
    ...
    if latest_event is None:
        ...
        if not isinstance(event_payload, Mapping) or event_payload.get("implementer") != implementer_profile or event_payload.get("reviewer") != reviewer_profile:
            errors.append("review_requested_event_missing_or_mismatched")
        raw_event_run_id = latest_event.get("run_id")
        if not isinstance(raw_event_run_id, int): errors.append("review_requested_event_run_id_required")
        else: event_run_id = raw_event_run_id
    ...
    if latest_run is None or latest_run.get("profile") != implementer_profile or latest_run.get("outcome") != "review_requested":
        ...
        if not isinstance(run_id, int): errors.append("current_implementer_run_id_required")
        elif event_run_id is None or run_id != event_run_id: errors.append("review_requested_event_run_mismatch")
        ...
        if not isinstance(metadata, Mapping):
            ...
            if metadata_path != resolved_path: errors.append("implementer_review_run_workspace_mismatched")
            if metadata.get("task_id") != task_id: errors.append("implementer_review_run_task_mismatched")
    return errors


def validate_routed_review_handoff(payload: Mapping[str, Any]) -> list[str]:
    route, route_errors = route_from_payload(payload)
    if route_errors or route is None: return [f"model_routing:{error}" for error in (route_errors or ["unparseable"])]
    if len(route.required_reviewers) != 1: return ["same_card_review_requires_exactly_one_reviewer"]
    return validate_review_handoff(payload, implementer_profile=route.implementer, reviewer_profile=route.required_reviewers[0])
...
def _json_object(value: str, label: str) -> Mapping[str, Any]:
    ...
    if not isinstance(parsed, dict): raise SystemExit(f"{label}: expected JSON object")
    return parsed
...
def _runtime_command(args: argparse.Namespace) -> int:
    ...
    errors = validate_runtime(actual, expectation); print(format_drift(errors)); return 0 if not errors else 2


def _routed_handoff_command(args: argparse.Namespace) -> int:
    actual = _json_object(args.actual_json, "actual-json")
    errors = validate_routed_review_handoff(actual); print(format_drift(errors)); return 0 if not errors else 2
...
def build_cli_parser() -> argparse.ArgumentParser:
    ...
    routed = sub.add_parser("routed-handoff"); routed.add_argument("--actual-json", required=True); routed.set_defaults(func=_routed_handoff_command)
    return parser


def main(argv: Sequence[str] | None = None) -> int:
    args = build_cli_parser().parse_args(argv); return int(args.func(args))
